# __PROJECT_NAME__ — System Architecture

## Overview

__PROJECT_NAME__ is structured as a layered ASP.NET Core application implementing a four-sided marketplace. The current implementation uses an in-memory store and is designed for incremental growth toward a persistent, event-driven architecture.
...
graph LR
    ...
    subgraph Api["__ROOT_NAMESPACE__.Api (Presentation)"]
        direction TB
    ...
    subgraph Core["__ROOT_NAMESPACE__.Core (Domain)"]
        direction TB
    ...
    subgraph Data["__ROOT_NAMESPACE__.Data (Infrastructure)"]
        direction TB
        DbCtx[__PROJECT_NAME__DbContext]
...
sequenceDiagram
    ...
    participant API as __ROOT_NAMESPACE__.Api
    participant RDS as RecipeDiscoveryService
    participant FS as FulfillmentService
    participant Store as __PROJECT_NAME__DbContext
...
|---|----------|--------|-------|
| ADR-001 | Use InMemoryStore for initial persistence | Superseded | Replaced by EF Core / `__PROJECT_NAME__DbContext` in `__ROOT_NAMESPACE__.Data` |
...
| ADR-004 | Introduce `__ROOT_NAMESPACE__.Data` for EF Core persistence | Accepted | Separates infrastructure from domain; Core remains framework-agnostic |
