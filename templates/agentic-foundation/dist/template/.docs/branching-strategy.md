# __PROJECT_NAME__ — Branching Strategy

__PROJECT_NAME__ follows the **Gitflow workflow** as defined by Atlassian:
https://www.atlassian.com/git/tutorials/comparing-workflows/gitflow-workflow

This model supports parallel agent development across isolated feature branches while keeping `master` stable for production releases.
...
- **Tagged**: every merge to `master` is tagged with a version (`v1.0.0`)
...
- **Requires**: human approval to merge to `master`

### `hotfix/<name>`
- **Branches from**: `master` (the specific release tag)
- **Merges into**: `master` AND `develop`
...
| `master` | `release/*` | Claude Code Review + build | Human required |
| `master` | `hotfix/*` | Claude Code Review + build | Human required |
...
sequenceDiagram
    ...
    Rel->>Human: PR to master — human approval required
...
- Every merge to `master` must be tagged
